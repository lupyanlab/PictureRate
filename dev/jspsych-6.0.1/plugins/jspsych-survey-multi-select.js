...
jsPsych.plugins['survey-multi-select'] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      min_choice: {
        ...
        pretty_name: 'Min Choice',
        default: null,
        description: 'Minimum Choices required.'
      },
      max_choice: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'Max Choice',
        default: null,
        description: 'Maximum Choices required.'
      }
    }
  }
  plugin.trial = function(display_element, trial) {
    ...
    var cssstr = ".jspsych-survey-multi-select-question { margin-top: 1em; margin-bottom: 1em; text-align: center; }"+
    ...
    trial_form.addEventListener('submit', function(event) {
      ...
      for(var index=0; index<matches.length; index++){
        ...
        if(val.length == 0 || !(val.length >= trial.min_choice) || !(val.length <= trial.max_choice)){ has_response.push(false); } else { has_response.push(true); }
      }
